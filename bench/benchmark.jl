...
# filename is stored in ARGS[1]

# print header info
open(ARGS[1], "w") do file
	println(file, "test_name,time_first_s,time_s,memory_B,restarts,miness")
	println(file, "metainfo,Time (First)<br>[s],Time<br>[s],Memory<br>[B],Restarts<br>,minESS<br>")
	println(file, "metainfo,lower,lower,lower,higher,higher")
end

benchmark(timefirst, time, mem, nrestart, miness, test_name::String) = open(ARGS[1], "a") do file
	println(file, "$test_name,$timefirst,$time,$mem,$nrestart,$miness")
end 
...
function benchmark(lambda::Function, test_name::String)
	...
	println("Running benchmark $test_name (first run)")
	first_result = @suppress_err @timed lambda(1)
	...
	# collect median time across 5 trials of precompiled run
	times = []
	mems = []
	restarts = []
	minesses = []
	for i=1:(dry_run ? 2 : 5)
		println("Running benchmark $test_name (subsequent run $i/5)")
		result = @suppress_err @timed lambda(i)
	...
	benchmark(first_result.time, median(times), median(mems), median(restarts), median(minesses), test_name)
end

using_result = @timed using Pigeons 
benchmark(0, using_result.time, using_result.bytes, 0, 0, "using Pigeons")
...
base_settings = (; show_report = true, record = [traces; round_trip; record_default()], effort...)
