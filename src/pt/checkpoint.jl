...
function PT(checkpoint_folder::String) 
    ...
    shared.exec_folder = fresh_exec_folder
...
function write_checkpoint(pt, reduced_recorders)
    if !pt.shared.input.checkpoint 
        return 
    end

    round = pt.shared.iterators.round
    checkpoint_folder = mkdir(pt.shared.exec_folder / "round=$round/checkpoint")
    only_one_process(pt) do
        ...
        if pt.shared.iterators.round == 1 
            serialize_immutables(pt.shared.exec_folder / "immutables.jls")
        end
    end
