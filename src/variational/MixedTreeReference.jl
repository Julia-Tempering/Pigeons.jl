...
variational_recorder_builders(::MixedTreeReference) = [_transformed_online_full]
...
function (variational::MixedTreeReference)(state)
    log_pdf = 0.0
...
LogDensityProblems.logdensity(log_potential::MixedTreeReference, x) =
    log_potential(x)
...
LogDensityProblemsAD.ADgradient(kind::ADTypes.AbstractADType, log_potential::MixedTreeReference, replica::Replica) = 
    BufferedAD(log_potential, replica.recorders.buffers)

function LogDensityProblems.logdensity_and_gradient(log_potential::BufferedAD{MixedTreeReference}, x)
