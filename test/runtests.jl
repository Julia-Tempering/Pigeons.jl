...
@assert pathof(Pigeons) == jointpath(project_root_dir, "src/Pigeons.jl")

# load all files starting with "test_"
for test_name in filter(x -> startswith(x, "test_") && endswith(x, ".jl"), readdir(".")) 
             # + - yes, we need this horror, because dealing with macros
             # v 
